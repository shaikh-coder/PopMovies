...
export const MoviesDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=ad13900422e30779212699bf67e6e9f9`);
    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          {movie.genres ? (
            <p className="my-4 flex flex-rap gap-2">
              {movie.genres.map((genre) => (
                <span className="mr-2 border rounded p-2 border-gray-200 dark:border-gray-600" key={genre.id}>{genre.name}</span>
              ))}
            </p>
          ) : ""}
